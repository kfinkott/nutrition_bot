...
class Sql_reports:
    today = datetime.now()
    def get_daily(self, table_name: str, weekday: str,
        ...
        try:
            ...
            return pd.DataFrame()
    def get_daily_png(self, table_name: str, weekday: str,
                      engine: sqlalchemy.engine.base.Engine) -> Union[os.PathLike, None]:
        """
        ...
        Returns:
        Union[os.PathLike, None]: Returns None if the dataframe is empty, 
          returns a file path to the image if the Dataframe has content
        """
        df_daily: pd.DataFrame = self.get_daily(table_name, weekday, engine)
        # if statement checks to see if the Dataframe is empty
        if df_daily.empty is False:
    ...
    def filter_days(self, df: pd.DataFrame, weekday: str) -> pd.DataFrame:
        ...
        pd.DataFrame: Dataframe
        """
        #selects the rows from the dataframe for the user selected day
        filter_str = df['weekday'].isin([weekday])
        return df[filter_str]
    def get_weekly(self, table_name: str,
                   engine: sqlalchemy.engine.base.Engine) -> pd.DataFrame:
        """
        ...
        return df_weekly
    def get_weekly_png(self, call: telebot.types.CallbackQuery,
                       table_name: str, week: str,
                       engine: sqlalchemy.engine.base.Engine) -> tuple[os.PathLike, pd.DataFrame]:
        """
        ...
        Returns:
        tuple[os.PathLike, pd.DataFrame]: Returns the file path for the
        report image and a Dataframe containing the report data
        """
        ...
        try:
            ...
            final_df.rename(columns={'breakfast': 'Breakfast', 'lunch': 'Lunch',
                                     'dinner': 'Dinner'}, inplace=True)
        ...
        # except block is for an unexpected use of this function.
        # a callback object could not be provided, so a raw string is used instead
        html_fn = 'default.html'
        try:
            if 'DD' in table_name:
        ...
        css = "body {background: #ffe0cc;} table {border: 1.3mm ridge #661a00;}\
td {width: 600;} td {font-size: 12px;} th {font-size:15px;} td {border: 1px solid black;}"
        final_img = nb_data_viz.export_html_to_png(call, html_fn, css, week)
        os.remove(html_fn)
        return final_img, final_df

    def get_daily_nutrition(self, table_name: str, weekday: str,
                            engine: sqlalchemy.engine.base.Engine, engine2: sqlalchemy.engine.base.Engine) -> pd.DataFrame:
        """
        ...
        table_name (str) : y
        weekday (str) : 
        engine (sqlalchemy.engine.base.Engine) : y
        engine2 ( _obj_ ) : 
        ...
        if daily_df_with_meals.empty is False:
            daily_sr = pd.concat([daily_df_with_meals['breakfast'],
                                  daily_df_with_meals['lunch'],
                                  daily_df_with_meals['dinner']]).dropna()
        else:
            daily_sr = pd.Series()
        if daily_sr.empty is False:
            stmt = 'select nl.name, sum(coalesce(dn.amount, 0)), \
sum(coalesce(dn.percentOfDailyNeeds, 0)) from ('
            for t in daily_sr:
                stmt += f'select name, amount, percentOfDailyNeeds from `{t}` union all '            
            stmt = stmt[:-11] + ') as dn RIGHT JOIN `nut_legend` nl ON \
dn.name=nl.name group by nl.name;'
    ...
    def get_weekly_nutrition(self, call, table_name: str, week: str,
                             engine: sqlalchemy.engine.base.Engine,
                             sql_auth) -> pd.DataFrame:
        """
        Creates a dataframe that contains the nutrient amount and daily value
        for each day of the week for all nutrients. Also contains an average
        for the week for both amount and DV
        ...
        Returns:
        pd.DataFrame: Dataframe containing a weekly summary of nutrients amount
        and Daily value
        """
    ...
    def get_weekly_nutrition_png(self, call, df: pd.DataFrame, week: str,
                                 engine: sqlalchemy.engine.base.Engine, title: str, sql_auth) -> os.PathLike:
        """
        Creates an image displaying a weekly report of nutrients amount and daily value.
        Also contains an average amount and daily value for the week
        
        Args:
        self ( _obj_ ) : class object
        call ( _obj_ ) : Telebot callback object
        df (pd.DataFrame) : Dataframe containing the nutrient data for amount and daily
          value
        week (str) : Week for which the report should be made
        engine (sqlalchemy.engine.base.Engine) : sQL alchemy engine
        ...
        Returns:
        os.PathLike: File path for the weekly nutrition report image
        """
        ...
        css = "tbody tr:nth-child(odd) {background-color: #dbbdbd;} body \
{background: #ffe0cc;} table {border: 1.3mm ridge #661a00;} td {width: 5.3%;} \
td {font-size: 12px;} th {font-size:15px;} td {border: 1px solid black;} \
th {border: 1px solid black;} table {border-collapse: collapse;} table {table-layout: fixed}"
        final_img = nb_data_viz.export_html_to_png(call, html_fn, css, week)
        os.remove(html_fn)
        return final_img
    def get_nutrients_legend_png(self, sql_auth) -> str:
        """
        Produces a nutrient legend image that indicates the measurement units
        for each nutrient that is contained in the daily value data
    ...
    def kevin_divide(self, x: float, y: float):
        ...
        Returns:
        Union[float, np.nan]: Returns np.nan if the divisor is zero. Returns a
        percentage otherwise
        """
        if y > 0:
            return x/y*100
        return np.nan # Returns nan if the divisor is zero
    def get_compare_diary_plan_weekly_png(self, call: telebot.types.CallbackQuery,
                            diary_table_name: str, plan_table_name: str, week: str,
                            enginediary: sqlalchemy.engine.base.Engine,
                            engineplan: sqlalchemy.engine.base.Engine,
                            sql_auth) -> tuple[os.PathLike, pd.DataFrame]:
        """
        Creates and image that shows a comparison between diary and diet plan for 
        a specified week
        ...
        enginediary (sqlalchemy.engine.base.Engine) : SQL alchemy engine for the diary 
          database
        engineplan (sqlalchemy.engine.base.Engine) : SQL alchemy engine for the diet plan 
          database
        
        Returns:
        tuple[os.PathLike, pd.DataFrame]: Returns a file path for the report image and a 
          dataframe containing the comparison data
        """
        pd.options.display.float_format = '{:.2f}'.format
        diary_df = self.get_weekly_nutrition(call, diary_table_name, week, enginediary,
                                             sql_auth)
        plan_df = self.get_weekly_nutrition(call, plan_table_name, week, engineplan,
                                            sql_auth)
        ...
        df_computed.rename(columns={'Sunday amount': 'Sunday % of diet plan',
                                    'Monday amount': 'Monday % of diet plan',
                                    'Tuesday amount': 'Tuesday % of diet plan',
                                    'Wednesday amount': 'Wednesday % of diet plan',
                                    'Thursday amount': 'Thursday % of diet plan',
                                    'Friday amount': 'Friday % of diet plan',
                                    'Saturday amount': 'Saturday % of diet plan'},
                           inplace=True)
        html_fn = f'dataviz/weekly_nut_compare_{call.from_user.id}.html'
        df_styled = Styler(df_computed,
                           caption=f'Week {week} Diary vs. Diet plan comparison')
        df_styled.set_table_styles([{
         'selector': 'td',
         'props': 'width:50px;'},{
         'selector': 'th scope="row"',
        ...
        df_styled.to_html(html_fn)
        css = "tbody tr:nth-child(odd) {background-color: #dbbdbd;} \
body {background: #ffe0cc;} table {border: 1.3mm ridge #661a00;} td {width: 5.3%;} \
td {font-size: 12px;} th {font-size:15px;} td {border: 1px solid black;} \
th {border: 1px solid black;} table {border-collapse: collapse;} table {table-layout: fixed}"
        final_img = nb_data_viz.export_html_to_png(call, html_fn, css, week)
        os.remove(html_fn)
    ...
    def group_weekdays_in_df(self, meal: pd.Series) -> str:
        """
        Combines the data for all meals in a single day (breakfast, lunch 
        and dinner combined)

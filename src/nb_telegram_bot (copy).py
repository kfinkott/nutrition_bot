...
def create_bot(bottoken: str, spoon_token: str, sql_auth: dict):
    """
    Root function for the main loop of the bot. Creates the telegram bot and at
    the end of the function initiates the polling loop. Contains all the code 
    logic for the bot within functions, classes and IF/ELIF/ELSE blocks.
    
    Args:
    bottoken (str) : Api key for the telegram bot API
    spoon_token (str) : Api key for the Spoonacular API
    sql_auth (dict) : Contains the address, username and port for the SQL 
    server.  Also contains an encrypted key for the SQL password.
    ...
    def send_food(message: telebot.types.Message, amount: int, unit: str, bot: telebot.TeleBot):
        ...
        the form of an image via the Telegram bot./
        
        Args:
        message (telebot.types.Message) : JSON information retrieved from the
          Telegram API via getupdates()
        amount (int) : Quantity of the simple food to be inquired about
        unit (str) : Measurement unit type for the quantity of food
        bot (telebot.TeleBot) : Telebot API class object. Contains methods to 
    ...
    def get_ing_photos(df1: pd.DataFrame, df2: pd.DataFrame, df_title: list)-> os.PathLike:
        """
        Function that retrieves two seperate images that are made from the 
        contents of a single dataframe. THe images contain nutritional 
        information for a simple food. Both images are concatenated within 
        this function to produce a single image which has both original 
        images side by side
        
        Args:
        df1 (pd.DataFrame) : Pandas Dataframe that contains nutritional 
          data for a simple food
        df2 (pd.DataFrame) : Pandas Dataframe that contains nutritional 
          data for a simple food
        df_title (list) : List of titles to be used at the top of the images
        
        Returns:
        os.PathLike: File path that points to the completed image to be sent to the 
          user
        """
    ...
    def get_nutrient_photo(nutrient_result: list, start_indx: int = 0) -> os.PathLike:
        """
        Function that retrieves two seperate images that are made from the 
        contents of a single dataframe. THe images contain food that contains a
        specified nutrient. Both images are concatenated within this function 
        to produce a single image which has both original images side by side
        
        Args:
        nutrient_result (list) : List of foods that contain the specified 
          nutrient
        start_indx (int=0) : Indicates where in the list to begin retrieving 
          results from. Used primarily when the user wants to view additional 
          results after receiving the first batch of results
        
        Returns:
        os.PathLike: File path that points to the completed image to be sent to the 
          user
        """
        df = pd.DataFrame(nutrient_result[start_indx:start_indx + 100], columns=[
                          'Food name', 'Amount', 'Unit', 'Nutrient'])
        df_title = [df.iloc[0, 3], 'shorecode.org']
        df1img = nb_data_viz.export_nut_search_png(
            df[:50], df_title, page2=False)
        df2img = nb_data_viz.export_nut_search_png(
            df[50:], df_title, page2=True)
    ...
    def send_nutrient(call: telebot.types.CallbackQuery, nutrient_id: str, start_indx: int = 0):
        ...
        Args:
        call (telebot.types.CallbackQuery) : JSON information retrieved from 
          the Telegram API via getupdates()
        nutrient_id (str) : Numeric ID that represents a specific nutrient
        start_indx (int = 0) : Indicates where in the list to begin retrieving 
          results from. Used primarily when the user wants to view additional 
          results after receiving the first batch of results
    ...
    def week_nut_diary(call: telebot.types.CallbackQuery, week: str):
        ...
        Args:
        call (telebot.types.CallbackQuery) :  JSON information retrieved from 
          the Telegram API via getupdates(). Contains the callback data sent 
          by the inline keyboard
        week (str) : The numeric week (out of 52) that the user specifies. 
          Indicates which section of teh diary to process and send to the user
...
_week{str(week)}_year{str(this_year)}'
    ...
    def week_nut_plan(call: telebot.types.CallbackQuery, week: str):
        ...
        user via the Telegram bot.
        
        Args:
        call (telebot.types.CallbackQuery) :  JSON information retrieved from 
          the Telegram API via getupdates(). Contains the callback data sent by
          the inline keyboard
        week (str) : The numeric week (out of 52) that the user specifies. 
          Indicates which section of teh diary to process and send to the user
...
_week{str(week)}_year{str(this_year)}'
            ...
            nb_api.dfplan = sql_reports.get_weekly_nutrition(
                call, table_name, week, engine, sql_auth)
            fn = sql_reports.get_weekly_nutrition_png(
    ...
    def comparison_file_export(userid: int, df: pd.DataFrame) -> tuple[str, str, str, str]:
        ...
        df (pd.DataFrame) : Pandas dataframe containing comparison data for 
          diet plan vs diary
    ...
    def callback_logic(call: telebot.types.CallbackQuery):
        ...
        if 'add_diary' in choice or 'add_plan' in choice:
            passwd = nb_sql_tasks.decrypt_passwd(
                'KEVIN_SQL', sql_auth['password_key'])
...
desired nutrient search:")
        ...
        if 'weekday_plan' in choice:
            table_name = f'{call.from_user.first_name}_PP_{str(call.from_user.id)}\
_week{str(this_week)}_year{str(this_year)}'
            passwd = nb_sql_tasks.decrypt_passwd(
                'KEVIN_SQL', sql_auth['password_key'])
            engine = nb_sql_tasks.engine(sql_auth, passwd, 'nb_diet_plan')
...
two digit week number of the year):', reply_markup=mk.plan_choose_week())
        ...
        if 'week_diary' in choice:
            ...
            try:
                table_name = f'{call.from_user.first_name}_DD_{str(call.from_user.id)}\
_week{str(choice[10:])}_year{str(this_year)}'
                passwd = nb_sql_tasks.decrypt_passwd(
                    'KEVIN_SQL', sql_auth['password_key'])
                engine = nb_sql_tasks.engine(sql_auth, passwd, 'nb_diary')
        ...
        if 'week_plan' in choice:
            ...
            try:
                table_name = f'{call.from_user.first_name}_PP_{str(call.from_user.id)}\
_week{str(choice[9:])}_year{str(this_year)}'
                passwd = nb_sql_tasks.decrypt_passwd(
                    'KEVIN_SQL', sql_auth['password_key'])
                engine = nb_sql_tasks.engine(
        ...
        if 'show_comparison_weekly' in choice:
            ...
            engineplan = nb_sql_tasks.engine(
                sql_auth, passwd, 'nb_diet_plan')
            week = choice[22:]
            week_nut_diary(call, week)
            week_nut_plan(call, week)
            diary_table_name = f'{call.from_user.first_name}_DD_{str(call.from_user.id)}\
_week{str(week)}_year{str(this_year)}'
            plan_table_name = f'{call.from_user.first_name}_PP_{str(call.from_user.id)}\
_week{str(week)}_year{str(this_year)}'
        ...
        if 'clear_diary' in choice:
            if len(choice) > 11:
                passwd = nb_sql_tasks.decrypt_passwd(
                    'KEVIN_SQL', sql_auth['password_key'])
                engine = nb_sql_tasks.engine(sql_auth, passwd, 'nb_diary')
                table_name = f'{call.from_user.first_name}_DD_{call.from_user.id}_week{this_week}\
_year{this_year}'
        ...
        if 'clear_plan' in choice:
            if len(choice) > 10:
                ...
                engine = nb_sql_tasks.engine(
                    sql_auth, passwd, 'nb_diet_plan')
                table_name = f'{call.from_user.first_name}_PP_{call.from_user.id}\
_week{this_week}_year{this_year}'
...
want to search by:', reply_markup=mk.recipe_search_choice())
        ...
        if choice == 'diary_auto_weekly':
            ...
            nb_sql_tasks.add_weekly_cron_sub(
                call.from_user.id, call.from_user.first_name, engine)
        if choice == 'plan_auto_weekly':
            passwd = nb_sql_tasks.decrypt_passwd(
                'KEVIN_SQL', sql_auth['password_key'])
            engine = nb_sql_tasks.engine(
                sql_auth, passwd, 'nb_diet_plan_cron')
            nb_sql_tasks.create_weekly_cron_tables(engine)
            nb_sql_tasks.add_weekly_cron_sub(
                call.from_user.id, call.from_user.first_name, engine)
...
_week{str(this_week)}_year{str(this_year)}'
            plan_table_name = f'{call.from_user.first_name}_PP_{str(call.from_user.id)}\
_week{str(this_week)}_year{str(this_year)}'
            compare_img_fn, compare_df = sql_reports.get_compare_diary_plan_weekly_png(
                call, diary_table_name, plan_table_name, str(this_week), enginediary,
                engineplan, sql_auth)
            compare_csv, compare_xlsx, compare_html, compare_json = comparison_file_export(
                call.from_user.id, compare_df)
            diary_img_fn, diary_df = sql_reports.get_weekly_png(
                call, diary_table_name, str(this_week), enginediary)
            plan_img_fn, plan_df = sql_reports.get_weekly_png(
                call, plan_table_name, str(this_week), engineplan)
            diary_csv, diary_xlsx, diary_html, diary_json = diary_file_export(
                call.from_user.id, compare_df)
            plan_csv, plan_xlsx, plan_html, plan_json = plan_file_export(
                call.from_user.id, compare_df)
            diary_nutrient_chart_fn = nb_data_viz.nutrient_report_chart(
                call, diary_table_name, this_week, enginediary, sql_auth)
            plan_nutrient_chart_fn = nb_data_viz.nutrient_report_chart(
                call, plan_table_name, this_week, engineplan, sql_auth)
            diary_weekday_chart_fn = nb_data_viz.weekly_dv_chart(
                call, diary_table_name, this_week, enginediary, sql_auth)
            plan_weekday_chart_fn = nb_data_viz.weekly_dv_chart(
                call, plan_table_name, this_week, engineplan, sql_auth)
            files = [nut_legend, compare_img_fn, compare_csv, compare_xlsx,
                     compare_html, compare_json, diary_csv, diary_xlsx,
                     diary_html, diary_json, plan_csv, plan_xlsx, plan_html,
    ...
    def main_callback(call: telebot.types.CallbackQuery):
        ...
        Args:
        call (telebot.types.CallbackQuery) :  JSON information retrieved from 
          the Telegram API via getupdates(). Contains the callback data sent by the inline keyboard
    ...
    def fire_ingredient(message: telebot.types.Message):
        ...
        Args:
        message (telebot.types.Message) :  JSON information retrieved from teh 
          Telegram API via getupdates()y
 ...
 be saved on our server.")        
    ...
    def default(message: telebot.types.Message):
        ...
        if message.chat.id in nb_api.ing_search_user_list:
            ...
            nb_api.diary_flag or message.from_user.id in nb_api.plan_flag or
            message.from_user.id in nb_api.diary_nut_flag or message.from_user.id
            in nb_api.plan_nut_flag):
            if len(message.text) == 1:
                message.text = '0' + message.text
            if message.from_user.id in nb_api.diary_flag:
                nb_api.diary_flag.remove(message.from_user.id)
                try:
                    table_name = f'{message.from_user.first_name}_DD_{str(message.from_user.id)}\
_week{message.text}_year{str(this_year)}'
                    passwd = nb_sql_tasks.decrypt_passwd(
                        'KEVIN_SQL', sql_auth['password_key'])
                    engine = nb_sql_tasks.engine(
            ...
            if message.from_user.id in nb_api.plan_flag:
                ...
                try:
                    table_name = f'{message.from_user.first_name}_PP_{str(message.from_user.id)}\
_week{message.text}_year{str(this_year)}'
                    passwd = nb_sql_tasks.decrypt_passwd(
                        'KEVIN_SQL', sql_auth['password_key'])
                    engine = nb_sql_tasks.engine(

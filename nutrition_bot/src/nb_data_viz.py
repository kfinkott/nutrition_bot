...
def export_png(df: pd.DataFrame, df_title: list, page2: bool=False) -> os.PathLike:
    """
    ...
    Returns:
    os.PathLike: Returns a string representing the file path of the image
    """
...
def export_nut_search_png(df: pd.DataFrame, df_title: list, page2: bool=False) -> os.PathLike:
    """
     Creates a png image from a provided dataframe
...
def collate_photos(photo1: os.PathLike, photo2: os.PathLike) -> os.PathLike:
    """
    Combines two images into a single image. the images are placed side by side
    
    Args:
    photo1 (os.PathLike) : The file name of the first image
    photo2 (os.PathLike) : The file name of the second image
...
def collate_3photos(photo1: os.PathLike, photo2: os.PathLike, photo3: os.PathLike) -> os.PathLike:
    """
    ...
    photo1 (os.PathLike) : The file name of the first image
    photo2 (os.PathLike) : The file name of the second image
    photo3 (os.PathLike) : THe file name of the third image
    ...
    dst = Image.new('RGB', (im1.width + im2.width + im3.width, max(im1.height, im2.height,
                    im3.height)), color='white') # color=white == background color
    dst.paste(im1, (0, 0))
    dst.paste(im2, (im1.width, 0))
...
def export_html_to_png(call: telebot.types.CallbackQuery, html: os.PathLike, css: str,
                       week: str) -> os.PathLike:
    """
    Creates a png image from a provided HTML file
    
    Args:
    call (telebot.types.CallbackQuery) : Telebot callback object
    html (os.PathLike) : File name of the html file
...
def create_nutrients_units_legend(engine: sqlalchemy.Engine) -> pd.DataFrame:
    ...
    with engine.connect() as conn:
        stmt = 'select name,unit from `nut_legend`;'
...
def nutrient_report_chart(call: telebot.types.CallbackQuery, table_name: str, week: str,
                          engine: sqlalchemy.Engine, sql_auth) -> str:
    """
    ...
    # copies of the dataframe are made to be sliced later then concatenated.
    # this is done to remove undesirable rows that are in the middle
    ...
    try:
        final_df['Daily average DV'].plot(kind='barh', title='Weekly Average Nutrient Consumption', yticks=range(0, len(df.index)),
                                xticks=range(0,101,10), figsize=(15,9), ylabel='Nutrients', xlabel='% of DV', rot=20, fontsize=6,
                                color='Orange', antialiased=True,xlim=(0,100), grid=True, width=0.77) #width is the width of teh bars
    except IndexError:
        fig = plt.figure()        
...
def weekly_dv_chart(call: telebot.types.CallbackQuery, table_name: str, week: str,
                    engine: sqlalchemy.Engine, sql_auth) -> str:
    """
    ...
    final_df.plot(kind='bar', title='Daily Average Consumption For All Nutrients', xticks=range(0, len(final_df.index)),
                  yticks=range(0,101, 10), ylim=(0,100), figsize=(9,9), ylabel='% of DV', color='Orange', width = 0.85,
                  rot=90, antialiased=True).set_xticklabels(['Sun', 'Mon', 'Tue', 'Wed','Thu', 'Fri', 'Sat', 'Avg'])
     # if statements that use the table_name flag to determine if the table is a diary or diet plan
    if 'DD' in table_name:
